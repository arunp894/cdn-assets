            ...
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so || is_draft) {
                        ...
                        if(ui.item.variation_double.length > 0){
                            variation_double_render(ui.item, purchase_line_id)            
                        }else{
...
// on click sub category in category drawer
    ...
    function variation_double_render(variation_product,purchase_line_id = null){
        if(variation_product.variation_double.length == 1){
            const varation_double = variation_product.variation_double[0]
            pos_product_row(variation_product.variation_id,purchase_line_id,null,1,varation_double.id)
        }else{
            openDoubleProductModel()
            generateDoubleproduct(variation_product)
        } 
    }
    ...
    $(document).on('click', 'div.product_box', function() {
        ...
        if ($('input#location_id').val() == '') {
            ...
            const variation_product = products.data.find(e => e.variation_id == variation_id)
            if(variation_product.variation_double.length > 0){
                variation_double_render(variation_product)            
            }else{
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1,variation_double_id = null) {
        ...
        $('#pos_table tbody')
            ...
            .each(function() {
                ...
                if (
                    ...
                    row_v_d_id == variation_double_id &&
                    enable_sr_no !== '1' &&
                    !modifiers_exist &&
                    !is_added
                ) {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            ...
            data: {
                ...
                is_draft: is_draft,
                variation_double_id : variation_double_id
            },
            dataType: 'json',
            success: function(result) {
                if (result.success) {
...
function openDoubleProductModel() {
    document.getElementById('Double_modal').classList.remove('hidden');
    // document.getElementById('backdrop').classList.remove('hidden');
}

function closeDoubleProductModal() {
    document.getElementById('Double_modal').classList.add('hidden');
    // document.getElementById('backdrop').classList.add('hidden');
}

function generateDoubleproduct(varation_double){    
    const Element = document.getElementById('Double_modal').querySelector('[double]')
    Element.innerHTML = ''
    document.getElementById('Double_modal').querySelector('[product_name]').innerHTML = varation_double.name
    varation_double.variation_double.forEach(e => {        
        Element.insertAdjacentHTML('beforeend', `<button onclick="pos_product_row(${varation_double.variation_id},null,null,1,${e.id})" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">${e.name}</button>`);
    })
}
// Optional: Close on ESC key
document.addEventListener('keydown', function (event) {
    if (event.key === 'Escape') closeDoubleProductModal();
});

// Optional: Close on backdrop click
document.getElementById('backdrop').addEventListener('click', closeDoubleProductModal());
