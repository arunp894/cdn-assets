...
$(document).ready(function() {
    ...
    $(document).on('change', '.unit_type', function() {
        ...
        if ($(this).val() == 'Double') {
           tr_obj.find('.product_unit').show(300)
           tr_obj.find('.product_unit select').attr('required', true);           
           tr_obj.find('.product_unit input').attr('required', true);           

        }
        else{
            tr_obj.find('.product_unit').hide(300)     
            tr_obj.find('.product_unit select').attr('required', false);
            tr_obj.find('.product_unit input').attr('required', false);
            tr_obj.find('.product_unit select').val('');
            tr_obj.find('.product_unit input').val('');
...
function skupopupmodel(event){
    const barcodes = event.currentTarget.value.split('|')
    .filter(v => v !== undefined && v.trim() !== '');
    const formGroup = document.querySelector('.BarcodeModel #group-element')
    formGroup.innerHTML = '';
    // Create the new input element
    barcodes.forEach((element,index) => {
        createbarcodeelemet(formGroup,index,element)
    });
    $('div.BarcodeModel').modal('show');
}
function addremove(type,index){
    const formGroup = document.querySelector('.BarcodeModel #group-element')
    createbarcodeelemet(formGroup)    
}
function remove(){
    event.currentTarget.parentElement.remove()
}
function savebarcode(){
    const data =  Array.from(document.querySelectorAll('.BarcodeModel #group-element input[type="text"]'))
    .map(e => e.value)
    .filter(v => v !== undefined && v.trim() !== '');

    barcode_sku.value = data.join("|")
    $('div.BarcodeModel').modal('hide');

}
function createbarcodeelemet(formGroup,index = 0, value = ''){
    ...
    newGroup.className = 'group-element';

    // Create button element
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'tw-dw-btn tw-dw-btn-neutral tw-text-white';
    button.textContent = 'Remove';
    button.setAttribute('onclick', `remove()`);

    // Append input & button to the new group-element
    newGroup.appendChild(input);
    newGroup.appendChild(button);

    // Append the new group-element to the parent container
    formGroup.prepend(newGroup);
}
function opendoubledata(subindex){   
    VUE_varation_double.subindex = subindex
    VUE_varation_double.ensureCurrentDouble()
    $('div.varation_double').modal({
        backdrop: 'static', 
        keyboard: false,
        show: true
    });
...
if(document.getElementById('varation_double')){
    var VUE_varation_double = new Vue({
        el: '#varation_double',
        data: {
            double : [],
            temp : [],
        ...
        watch : {
            type : function (val){
                if(val === 'single' ){
                    this.double = this.temp
                }
                if(val === 'variable' ){
                    this.temp = this.double
                    this.double = []
                }
            },
            double : function (val) {
                if(this.type == 'single'){
                    if(this.ensureCurrentDouble() !== undefined && this.ensureCurrentDouble().length){
                        // document.getElementById('double_unit_total').innerHTML = `${this.ensureCurrentDouble().length} Double unit Added`
                    }else{
                        // document.getElementById('double_unit_total').innerHTML = ``
                    }
                }else{
        ...
        methods: {            
            saveandclose(){
                const isvalide = this.ensureCurrentDouble().find(e => e.name === '' || e.default_sell_price === '' || e.con_rate === '')
                if(isvalide){
                    alert('fill all fields ')
                }else{
                    $('div.varation_double').modal('hide')
                }                
            },
            ensureCurrentDouble(){
                let result = this.double.findIndex(innerArray => innerArray.id === this.subindex )
                ...
                this.current_index = this.double.findIndex(innerArray => innerArray.id === this.subindex )
                return this.double[this.current_index].double          
            },
            add(){
                const data = {
                    id : '',
                ...
                this.ensureCurrentDouble().push(data)
                this.ensureCurrentDouble()
            },
            remove(index){
                this.ensureCurrentDouble().splice(index, 1);
            },
            getdouble(){
                const result = [];
                for (const group of this.double) {
                    if (!Array.isArray(group.double)) continue;

                    for (const item of group.double) {
